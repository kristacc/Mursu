...
void uart_init(){
	...
	// enable transmitter, receiver and receiver interrupt. NOTHING ELSE, so no =&
	UCSR0B = _BV(RXEN0) | _BV(TXEN0) | _BV(RXCIE0);
	
	// data format: 8 bit, 1 stop bit, 	no parity (datasheet pg 183 - 184)
	// UCSR0C USART stayus control register C, used for dataformat configuration
...
void modbus_error_response( uint8_t func, uint8_t error ){
	uint16_t msg_crc = 0xFFFF; // modbus crc initial value before any bytes has been sent
...
void modbus_read_register( uint16_t start, uint16_t stop ){
	...
	for( addr = start; addr < stop; ++addr ){
		...
		uint16_t register_content = measurement_table[addr - modbus_start_address];
...
void modbus_write_register(uint16_t address, uint16_t data){
	...
	// TODO: add other functions, like changing base level of sensors:
...
void modbus_handle_message(){
	...
	// check function code (we support currently only "READ HOLDING REGISTER" and "WRITE SINGLE REGISTER" )
	...
	if( (g_uart_buffer[MODBUS_FUNC_CODE_IDX] != MODBUS_READ_HOLDING_REGISTER) && 
		(g_uart_buffer[MODBUS_FUNC_CODE_IDX] != MODBUS_WRITE_SINGLE_REGISTER)){
